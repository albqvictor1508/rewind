...
authRoutes.get(
  ...
  async (_, response) => {
    ...
    if (!url) return response.status(500).json("Error to generate Oauth Google URL");
...
authRoutes.get(
  ...
  async (_, response) => {
    ...
    if (!url) return response.status(500).json("Error to generate Oauth Github URL");
    return response.redirect(url);
  }
);
...
authRoutes.post(
  ...
  async (request, response) => {
    ...
    await Promise.all([
      mail({ to: email, subject: "Rewind | Seu código comédia", text: code }),
